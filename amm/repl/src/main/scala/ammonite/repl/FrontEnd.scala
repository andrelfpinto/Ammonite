...
//import org.jline.console.{ConsoleReader, completer}
...
object FrontEnd{
  ...
  class JLineTerm() extends FrontEnd {
    val makeTerm = () => TerminalBuilder.builder().build()
...
//                    .parser(parser)
      ...
      try for {
        _ <- Catching{ case e: org.jline.reader.UserInterruptException =>
          if (e.getPartialLine == "") terminal.writer().print("Ctrl-D to exit")
          Res.Skip
        }
        res <- readCode("")
      } yield res
...
//  class JLineTerm(makeTerm: () => org.jline.terminal.Terminal) extends FrontEnd{
//    def width = makeTerm().getWidth
//    def height = makeTerm().getHeight
//
//    def action(input: InputStream,
